...
prices, market_data = getStockDataVec(stock_name, timesteps)
...
for e in range(episode_count + 1):
    ...
    for t in range(l):
        action = agent.act(state)  # lets add position state later to the state, needed!, calculate restrictions there or here?, that's reason not to combine those yet, maybe next version... combined!
          # State could be just a list of [market_state, position_state, ....] and so on...
        ...
        #print("Eka", state[1])
        next_position_state, immediate_reward, PnL = getNextPositionState(action, state[1][0], prices[t], prices[t+1] )  # FYI, state[1] = position_state
